...
6. Import all ORM model modules in `migrations/env.py` before assigning
   `target_metadata`.
...
11. Add tests that run `alembic upgrade head` against a temp database and check
    for pending autogenerate changes.
