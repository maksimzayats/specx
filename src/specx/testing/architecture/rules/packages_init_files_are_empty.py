    ...
    def check(self, context: ArchitectureContext) -> tuple[SpecxArchitectureViolation, ...]:
        ...
        for package_root in (context.src_root, context.project_root / "tests"):
            if not package_root.exists():
                continue
            for directory in python_package_directories(context, root=package_root):
                init_path = directory / "__init__.py"
                if not init_path.exists():
                    violations.append(
                        violation(self.id, path=init_path, message="__init__.py is missing")
                    )
                    continue
                if init_path.read_text(encoding="utf-8") != "":
                    violations.append(
                        violation(self.id, path=init_path, message="__init__.py is not empty")
                    )
