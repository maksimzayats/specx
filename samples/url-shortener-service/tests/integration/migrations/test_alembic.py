...
def test_alembic_env_uses_project_local_sqlalchemy_base() -> None:
    text = (PROJECT_ROOT / "migrations" / "env.py").read_text(encoding="utf-8")
...
def test_alembic_migrations_create_schema(
    ...
    try:
        ...
        assert sorted(inspector.get_table_names()) == ["alembic_version", "short_urls"]
        columns = {column["name"] for column in inspector.get_columns("short_urls")}
        assert columns == {"id", "code", "target_url"}
    finally:
        engine.dispose()
